...
- `sb_drive COMMAND STEP…` plays keys into a TUI in the real window: it runs `zsh -i -c COMMAND` (`'ttheme preview'`, `'ttheme browse'`) on a pty of its own sized like the window, relays its output to the window and the window's replies back — so OSC colors, kitty graphics and cell-size queries all work — and takes the steps in order: a key name (`up` `down` `left` `right` `shift-left`… `home` `end` `pgup` `pgdn` `enter` `esc` `tab` `space` `bs` `ctrl-c` `ctrl-u` `ctrl-v` `alt-c` `alt-v` `focus-in` `focus-out`), a single character, `text:miku` typed one character at a time, `paste:TEXT` sent as one bracketed paste (what a terminal sends for a drop or Cmd+V), a number of seconds to wait (so a digit key goes as `text:9`), or `shot:NAME` (as `sb_shot`). Keys go 0.25 s apart, and the child is killed 10 s after the last step. The child's shell variables do not come back: read the outcome from files, `sb_var`, `sb_color` or a screenshot. The commands' own shell still believes in its old palette, so its next prompt may put that palette's picture back (one extra Ghostty reload) — the product never does this, since preview runs in the shell that wears the result.
  ```zsh
...
Keep screenshots in the session scratchpad, never the repo, and read them with the Read tool. The window is unfocused, so the cursor is drawn hollow.
...
The window's profile is `ttheme sandbox` (Guid `sandbox`, in `sandbox.json` next to `ttheme.json`), the suite's default — the harness sets it after `init`, which would otherwise have made `ttheme · default` the default. Every sandbox shell and the CLI it runs export `TTHEME_ITERM_SUITE=ttheme-sandbox`, so `init` and `ttheme default` write the suite's `Default Bookmark Guid`, never the user's. Because this default is a dynamic profile, iTerm2 writes it back over any outside change on the next dynamic-profile reload (a user's regular default does not do that); to see what a user gets after `ttheme default`, write `ttheme-default` into the suite again once the reload settles, quit the instance and relaunch it with the harness's `open` arguments without wiping. Its command sets `HOME` to the sandbox as well, so a `ttheme add` inside writes the sandbox's profiles, not the user's. The suite is seeded with the Python API on, a per-session cookie, and `ReportVariable` allowed for `id`, `tab.id`, `tab.window.id`, `profileName` and `user.ttheme_bg`.
...
- The Metal renderer flips the `y` of a kitty source rectangle (`x=0,y=0,w=200,h=100` on a red-over-blue image draws blue); `--legacy` draws it right. A placement that leaves the grid — negative `V` on a relative one, or one running past the bottom — is not drawn at all.
...
- A background image is reloaded only when the profile changes, and the image cache keys on the path (checked against its modification time), so a new picture under an old path stays invisible until something else in the profile changes.
- `Blend` is the image's opacity: 0 hides it, 1 shows it whole. It composites the same as preview's kitty layers (measured within two levels on flat white at 0.5), and transparent pixels come out as the background color.
