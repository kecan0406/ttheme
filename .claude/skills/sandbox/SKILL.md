...
- `sb_tab 'commands'` spends that call on a new tab whose first prompt runs the commands (`REPLY` is its number); `sb_tab_wait N` waits for it. A tab gets its own call, so it can switch back with `sb_it2 tab select 0 --window "$(sb_var tab.window.id)"` or open the next tab.
