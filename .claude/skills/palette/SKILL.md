...
- `meta.booru` is one tag searched on four sites that name characters differently. `bun tags.ts themes/<name>.toml [--try <tag>]` counts it on each `find` site (sequentially — safebooru.org answers 0 when hit in parallel); keep the tag that covers the most sites (Leafa is `leafa` on three sites and `leafa_(sao)` only on donmai).
