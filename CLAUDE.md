...
- `mise run sandbox [--ghostty [--behind]] [--zshenv FILE] [palette…]` — try ttheme by hand as a fresh user of this checkout: a throwaway home at `$TMPDIR/ttheme-sandbox`, wiped on every run and kept after exit for inspection. Plain mode drops into a zsh in this tab (`HOME` moved; `TTHEME_*`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME` and `ZDOTDIR` scrubbed); `--ghostty` opens a separate Ghostty instance on it, the only way to test `keep`, new tabs and config reloads; `--behind` hands focus back once its window exists; `--zshenv` seeds a hook every sandbox shell sources first. Claude drives it through the `sandbox` skill (`.claude/skills/sandbox/`)
